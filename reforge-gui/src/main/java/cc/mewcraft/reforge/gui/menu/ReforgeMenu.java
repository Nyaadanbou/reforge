...
import cc.mewcraft.reforge.gui.manager.ReforgeManager;
...
public class ReforgeMenu {
    ...
    private final String reforgeOptionKey;
    ...
    public ReforgeMenu(
            final Logger logger,
            @ReforgeMenuConfig final FileConfiguration config,
            final Translations translations,
            final ReforgeManager manager,
            @ReforgeOptionKey final String reforgeOptionKey
    ) {
        this.logger = logger;
        this.config = config;
        this.translations = translations;
        this.manager = manager;
        this.reforgeOptionKey = reforgeOptionKey;
        ...
        // Backed GUI
        Gui gui = Gui.normal()
                .setStructure(config.getStringList("gui.layout").toArray(String[]::new))
                .addIngredient('#', new SimpleItem(
                        new ItemBuilder(Material.BLACK_STAINED_GLASS_PANE)
                                .setDisplayName(translatable("menu.reforge.background.name"))
                ))
    ...
    private class ReforgeItem extends AbstractItem {
        ...
        @Override public void handleClick(final @NotNull ClickType clickType, final @NotNull Player player, final @NotNull InventoryClickEvent event) {
            ...
            if (clickType.isLeftClick()) {
                ...
                Optional<ItemStack> optional = ReforgeProvider.get().transform(item, reforgeOptionKey);
                if (optional.isEmpty()) {
